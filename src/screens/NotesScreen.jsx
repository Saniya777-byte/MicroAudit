...
export default function NotesScreen({ navigation }) {
  ...
  const NoteCard = ({ note }) => (
    <TouchableOpacity onPress={() => navigation.navigate("NoteEditor", note)} onLongPress={() => onLongPressNote(note)} style={[styles.card, { backgroundColor: note.color }]}> 
  ...
  const NoteRow = ({ note }) => (
    <TouchableOpacity onPress={() => navigation.navigate("NoteEditor", note)} onLongPress={() => onLongPressNote(note)} style={styles.row}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.rowPreview} numberOfLines={1}>{note.content}</Text>
      </View>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
        {allTags.map((t) => {
          ...
          return (
            <TouchableOpacity key={t} style={[styles.chip, active ? styles.chipActive : styles.chipOutline]} onPress={() => setActiveTag(t)} onLongPress={() => Alert.alert("Tag", t, [{ text: "Rename" }, { text: "Delete", style: "destructive" }, { text: "Cancel", style: "cancel" }])}>
              <Text style={[styles.chipText, active && { color: "#111827", fontWeight: "700" }]}>{t}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
      ...
      <View style={styles.viewToggle}>
        <TouchableOpacity style={[styles.toggleBtn, view === "grid" && styles.toggleActive]} onPress={() => setView("grid")}>
          <Grid2x2 size={16} color={view === "grid" ? colors.surface : colors.text} />
          <Text style={[styles.toggleText, view === "grid" && { color: colors.surface }]}>Grid</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.toggleBtn, view === "list" && styles.toggleActive]} onPress={() => setView("list")}>
          ...
          <Text style={[styles.toggleText, view === "list" && { color: colors.surface }]}>List</Text>
        </TouchableOpacity>
      </View>
      ...
      {/* Options Menu (simple placeholder) */}
      <Modal transparent visible={menuOpen} animationType="fade" onRequestClose={() => setMenuOpen(false)}>
        <Pressable style={styles.menuBackdrop} onPress={() => setMenuOpen(false)} />
        <View style={styles.menu}>
          <Text style={styles.menuTitle}>Options</Text>
          <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); }}><Text>Sort: Newest first</Text></TouchableOpacity>
          <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); setView("grid"); }}><Text>Change view: Grid</Text></TouchableOpacity>
          <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); setView("list"); }}><Text>Change view: List</Text></TouchableOpacity>
          <TouchableOpacity style={styles.menuItem} onPress={() => { setMenuOpen(false); }}><Text>Create Tag</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.menuItem, { borderTopWidth: 1, borderTopColor: "#E5E7EB" }]} onPress={() => { setMenuOpen(false); Alert.alert("Danger", "Delete all?", [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive" }]); }}><Text style={{ color: colors.danger }}>Delete all</Text></TouchableOpacity>
        </View>
      </Modal>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: spacing.md, paddingVertical: spacing.md, backgroundColor: colors.surface, shadowColor: "#000", shadowOpacity: 0.06, shadowRadius: 8, elevation: 2, borderBottomLeftRadius: radius.lg, borderBottomRightRadius: radius.lg },
  ...
  toggleBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 9999, backgroundColor: colors.surface, borderWidth: 1, borderColor: "#E5E7EB" },
  toggleActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  toggleText: { fontSize: fonts.small, color: colors.text },
  sectionLabel: { marginTop: spacing.md, marginBottom: spacing.sm, marginLeft: spacing.md, fontWeight: "700", color: colors.muted },
  ...
  row: { flexDirection: "row", alignItems: "center", paddingHorizontal: spacing.md, paddingVertical: 12, backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: "#E5E7EB" },
  ...
  menu: { position: "absolute", top: 60, right: 12, backgroundColor: colors.surface, padding: spacing.md, borderRadius: radius.md, width: 220, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 10, elevation: 4 },
